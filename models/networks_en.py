...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt,skip, bilinear=True):
    ...
    def forward(self, input, gray):
        ...
        input_gray=torch.cat((input, gray), 1)
        x = input_gray
        x1 = self.inc(x) 
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        ...
        x = self.up1(x5, x4) * gray_4 if self.opt.self_attention else self.up1(x5, x4)
        x = self.up2(x, x3) * gray_3 if self.opt.self_attention else self.up2(x, x3) 
        x = self.up3(x, x2) * gray_2 if self.opt.self_attention else self.up3(x, x2)
        x = self.up4(x, x1) * gray if self.opt.self_attention else self.up4(x, x1)

        latent = self.outc(x)
        if self.opt.uncer:
                uncer = F.softplus(self.outc(x))
                output = latent + input*uncer
        else:
                output = latent + input

        return output, latent 

class Unet_resize_conv_m(nn.Module):
    ...
    def forward(self, input, gray):
        ...
        if self.opt.use_norm == 1:
            ...
            if self.opt.uncer:
                uncer = F.softplus(self.LReLU1_1(self.bn11(self.conv11(conv9))))
